...
class MicrophoneCapture:
    ...
    def start(self):
        ...
        # 1) Listen for mic device running state
        ...
        # 2) Listen for audio process list changes
        _add_property_listener(
            kAudioObjectSystemObject, self._proclist_prop, self._proclist_listener
        )

        # 3) Register per-process listeners for all current audio processes
    ...
    def stop(self):
        ...
        # Remove system process list listener
        _remove_property_listener(
            kAudioObjectSystemObject, self._proclist_prop, self._proclist_listener
        )
    ...
    # ============================================================
    # Callback 1: Device running state (recording trigger)
    # ============================================================
    ...
    def _sync_process_listeners(self):
        ...
        # Removed processes — unregister listeners
    ...
    # ============================================================
    # Callback 3: Per-process input state changed (join/leave)
    # ============================================================
    ...
    def _on_process_input_changed(self, obj_id, num_addr, addresses, client_data):
        ...
        # Check if this process is now using our mic device
    ...
    def _debounced_start(self):
        if not self._running:
            return
        if _is_device_running(self._device_id) and not self._recording:
            self._start_recording()

    def _debounced_stop(self):
        if not self._running:
            return
        if not _is_device_running(self._device_id) and self._recording:
            self._stop_recording()
    ...
    def _get_mic_clients(self) -> dict[str, str]:
        """Return {bundle_id: app_name} for processes using the default input device."""
    ...
    def _start_recording(self):
        ...
        first_bundle = sorted(clients.keys(), key=lambda b: clients[b])[0]
        self._primary_app = clients[first_bundle]
        self._primary_bundle = first_bundle

        # Log mic_start
        self._log_event("mic_start", self._format_clients(clients))
    ...
    def _audio_callback(self, indata, frames, time_info, status):
        """sounddevice InputStream callback — runs on PortAudio thread."""
        if status:
            print(f"[Mic] Audio status: {status}")
        self._audio_queue.put(indata.copy())

    def _audio_writer(self):
        """Writer thread: drains audio queue and writes to WAV."""
